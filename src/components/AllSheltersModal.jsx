  ...
  const loadAllShelters = async () => {
    try {
      ...
      try {
        ...
        throw new Error("위치 정보 접근이 필요합니다. 브라우저에서 위치 권한을 허용해주세요.");
      }

      const serviceKey = import.meta.env.VITE_PUBLIC_DATA_SERVICE_KEY;
      if (!serviceKey) {
        throw new Error(
          "공공데이터 서비스키가 설정되지 않았습니다."
        );
      }
        ...
        .map((shelter) => {
          ...
          return {
            ...
            operatingHours: (() => {
              ...
              const wkendBegin = pickFirst(shelter, ["WKEND_HDAY_OPER_BEGIN_TIME"]);
              const wkendEnd = pickFirst(shelter, ["WKEND_HDAY_OPER_END_TIME"]);
              
              if (wkdayBegin && wkdayEnd) {
                const wkday = `${wkdayBegin.slice(0, 2)}:${wkdayBegin.slice(2, 4)} ~ ${wkdayEnd.slice(0, 2)}:${wkdayEnd.slice(2, 4)}`;
                if (wkendBegin && wkendEnd) {
                  const wkend = `${wkendBegin.slice(0, 2)}:${wkendBegin.slice(2, 4)} ~ ${wkendEnd.slice(0, 2)}:${wkendEnd.slice(2, 4)}`;
      ...
      <div className="relative bg-white rounded-xl shadow-xl border border-gray-200 w-[90vw] max-w-3xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b gap-3">
          <div className="text-base font-bold text-gray-800">
            전국 무더위 쉼터 {allShelters.length ? `(${allShelters.length.toLocaleString()}개)` : ''}
          </div>
        ...
        <div className="p-4 overflow-auto">
          {loadingAll ? (
            <div className="py-10 text-center text-sm text-gray-500">전체 목록 불러오는 중...</div>
          ) : allError ? (
            <div className="py-10 text-center text-sm text-red-500">{allError}</div>
          ) : allShelters.length === 0 ? (
            <div className="py-10 text-center text-sm text-gray-500">표시할 쉼터가 없습니다.</div>
          ) : (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      {s.address && <div className="text-xs text-gray-500 mt-1">{s.address}</div>}
                      {s.typeName && <div className="text-xs text-blue-600 mt-1">{s.typeName}</div>}
                      {s.location && <div className="text-xs text-gray-400 mt-1">{s.location}</div>}
                    </div>
                    <div className="text-sm text-gray-500 ml-2">{s.distance}</div>
                  </div>
                </li>
              ))}
            </ul>
