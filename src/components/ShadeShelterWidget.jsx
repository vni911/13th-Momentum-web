    ...
    const fetchShelters = async () => {
      try {
        ...
        const shelterList = shelters
          ...
          .slice(0, 4) // 상위 4개만 선택
          .map(({ name, address, distance, type, location }) => ({ name, address, distance, type, location })) // actualDistance 제거
             ...
             <div key={index} className={`py-3 px-4 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors duration-200 border border-gray-200 ${index < shelters.length - 1 ? 'mb-3' : ''}`}>
               <div className="flex justify-between items-start">
                 <div className="flex-1">
                   ...
                   {shelter.location && (
                     <p className="text-xs text-gray-400 mt-1">{shelter.location}</p>
                   )}
                 </div>
                 <span className="text-sm text-gray-500 ml-2">{shelter.distance}</span>
               </div>
             </div>
           ))
        )}
